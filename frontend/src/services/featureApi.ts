...
export async function getAdminStats(){if(DEMO)return adminStats;const x=await call<any>("/admin/analytics");return {totalUsers:x.users,activeUsers:x.activeUsers??x.users,totalRides:x.rides,completedRides:x.completedRides??0,cancelledRides:x.cancelledRides??0,totalPassengers:x.bookings,avgRating:x.avgRating??0,monthlyFuelSaved:x.monthlyFuelSaved??0}}
export async function getRouteStats(){if(DEMO)return routeStats;const x=await call<any[]>("/admin/analytics/routes");const total=x.reduce((n,r)=>n+Number(r.rides||0),0)||1;return x.map(r=>({...r,share:Number(r.rides||0)/total}))}
...
export async function checkInPassenger(rideId:string,booking:Booking,code:string):Promise<CheckIn>{
 if(!DEMO)return call<CheckIn>("/checkins",{method:"POST",body:JSON.stringify({rideId,bookingId:booking.id,code})});
...
export async function fetchRideCheckIns(rideId:string){return DEMO?getRideCheckIns(rideId):call<CheckIn[]>(`/checkins/${rideId}`)}
export function getDriverRideCode(rideId:string){return getRideCode(rideId)}
export async function fetchDriverRideCode(rideId:string){if(DEMO)return getRideCode(rideId);const x=await call<{code:string}>(`/rides/${rideId}/code`);return x.code}
...
export async function joinWaitlist(rideId:string,seats=1):Promise<WaitlistEntry>{
 if(!DEMO)return call<WaitlistEntry>("/waitlist",{method:"POST",body:JSON.stringify({rideId,seats})});
...
export async function fetchRideWaitlist(rideId:string){return DEMO?getRideWaitlist(rideId):call<WaitlistEntry[]>(`/waitlist/${rideId}`)}
...
export async function fetchRidePreferences(rideId:string){return DEMO?getPreferences(rideId):call<RidePreferences>(`/rides/${rideId}/preferences`)}
export function setRidePreferences(rideId:string,prefs:RidePreferences){if(!DEMO)return call<RidePreferences>(`/rides/${rideId}/preferences`,{method:"PUT",body:JSON.stringify(prefs)});savePreferences(rideId,prefs);return prefs}
export function getTrusted(){return getTrustedContact()}
export async function fetchTrusted(){return DEMO?getTrustedContact():call<TrustedContact|null>("/safety/trusted-contact")}
export function saveTrusted(contact:TrustedContact){if(!DEMO)return call<TrustedContact>("/safety/trusted-contact",{method:"PUT",body:JSON.stringify(contact)});saveTrustedContact(contact);return contact}
export function getImpact(){return getSharedImpact()}
export async function fetchImpact(){return DEMO?getSharedImpact():call<{sharedTrips:number;fuelSaved:number;moneySaved:number;co2:number}>("/impact")}
