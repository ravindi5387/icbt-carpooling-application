...
import {reviewJoinRequest,fetchDriverRideCode,fetchRideCheckIns,startRide,completeRide} from "../services/featureApi";
import {SectionTitle,Metric} from "../components/FeatureShell";
import {Link} from "react-router-dom";
import {getDriverRides,getDriverRequests,getDriverRideBookings,getRide} from "../services/api";
...
export default function DriverDashboard(){
 ...
 async function sync(){
  const [rides,reqs]=await Promise.all([getDriverRides(),getDriverRequests()]);
  setMyPostedRides(rides); setRequests(reqs as any);
 }
 useEffect(()=>{sync().catch(e=>window.alert(e instanceof Error?e.message:"Could not load driver data."))},[user?.id]);
 function syncRequests(){sync().catch(()=>{})}
...
function PassengerDetails({rideId}:{rideId:string}){
 const [bookings,setBookings]=useState<any[]>([]); const [checks,setChecks]=useState<any[]>([]);
 useEffect(()=>{Promise.all([getDriverRideBookings(rideId),fetchRideCheckIns(rideId)]).then(([b,c])=>{setBookings(b);setChecks(c)}).catch(()=>{})},[rideId]);
...
function RideControl({rideId}:{rideId:string}){
 ...
 useEffect(()=>{getRide(rideId).then(setRide)},[rideId]);
 if(!ride)return <div className="state"><h3>Loading ride…</h3></div>;
 const [checks,setChecks]=useState<any[]>([]); const [code,setCode]=useState("••••");
 useEffect(()=>{fetchRideCheckIns(rideId).then(setChecks).catch(()=>{});fetchDriverRideCode(rideId).then(setCode).catch(()=>{})},[rideId]);
 ...
 useEffect(()=>{getDriverRideBookings(rideId).then(setPassengers).catch(()=>{})},[rideId]);
