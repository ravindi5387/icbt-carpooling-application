...
import {getBookings,cancelBooking,getRides} from "../services/api";
import {SectionTitle} from "../components/FeatureShell";
import {Link} from "react-router-dom";

export default function Bookings(){
 const [bookings,setBookings]=useState<Booking[]>([]); const [rides,setRides]=useState<any[]>([]);
 const [loading,setLoading]=useState(true);
 const [busy,setBusy]=useState<string|null>(null);
 const [activeTab,setActiveTab]=useState<"all"|"requested"|"confirmed"|"rejected">("all");
 useEffect(()=>{Promise.all([getBookings(),getRides({origin:"",destination:"",date:"",time:"",seats:"",maxPrice:"",sort:"time"})]).then(([b,r])=>{setBookings(b);setRides(r)}).finally(()=>setLoading(false))},[]);
 ...
 return <div className="page">
  ...
  <div className="booking-list">
   {loading?<div className="panel empty-state">Loading your bookings…</div>:filtered.length===0?
    ...
    :filtered.map(b=>{
      const r=rides.find(x=>x.id===b.rideId); if(!r)return null;
